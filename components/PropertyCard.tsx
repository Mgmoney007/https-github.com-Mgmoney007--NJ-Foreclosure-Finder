...
const PropertyCard: React.FC<PropertyCardProps> = ({ property, onClick }) => {
  ...
  return (
    <div 
      onClick={onClick}
      className={`bg-white rounded-xl shadow-sm border border-slate-200 border-t-4 ${riskBorderColor} hover:shadow-lg hover:-translate-y-1 transition-all duration-200 cursor-pointer overflow-hidden flex flex-col h-full`}
    >
      {/* Header */}
      <div className="p-4 border-b border-slate-100 flex justify-between items-start bg-slate-50/50">
        <div className="overflow-hidden">
          <h3 className="font-bold text-slate-900 truncate text-lg" title={address.full}>
            {address.street}
          </h3>
          <p className="text-sm text-slate-500 truncate">{address.city}, {address.state} {address.zip}</p>
        </div>
        <span className={`flex-shrink-0 px-2.5 py-1 rounded-full text-xs font-bold border ${riskColorClass}`}>
      ...
      <div className="p-4 flex-1 space-y-5">
        ...
        <div className="flex items-center justify-between bg-slate-50 p-3 rounded-lg border border-slate-100 relative overflow-hidden">
            ...
            <div className="flex items-center justify-center -mt-6 -mr-4 transform scale-75 origin-top-right">
